...
var getValue = function(name, c, res) {
		if (c.type === 'redirect') {
			...
			dnsAnswer(res,[{type:Consts.NAME_TO_QTYPE.A,address:c.value,name:name}]);
		}
	};
 ...
 *	value Array[String]
 */
var dnsAnswer = function(res, value) {
		if (util.isArray(value)&&value.length>0) {
			logger.debug(util.inspect(value));
...
logger.info('dns server start on 53');
