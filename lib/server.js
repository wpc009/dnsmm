...
var getValue = function(query, c, res) {
		if (c.type === 'redirect') {
			forward(res, {"from":query.name,"to":c.value},query.type);
		} else {
			dnsAnswer(res,[dns.A({type:Consts.NAME_TO_QTYPE.A,address:c.value,name:query.name,ttl:ttl,})]);
		}
	};
...
var forward = function(res,data ,qtype) {
	var req = dns.Request({
	...
	req.on('timeout',function(){
		logger.error('query %s to upstream timeout', data.from);
	//	res.cancel();
		res = null;
	});
...
/*
	...
	req.on('end',function(){
		logger.debug('req end for ', data.from);
		req = null;
	});
	req.send();
};
